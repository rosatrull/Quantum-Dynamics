...
# i labels the center of the excitation, not a particle.
i_assigned = 25  # center site
i = i_assigned - 1  # zero-based

eta = 1j  # choose from 1, -1, 1j, -1j
...
# ↑ component
Gamma[i][:,:,0] = np.sqrt(2) * np.eye(2)  # [[√2, 0], [0, √2]]

# ↓ component
Gamma[i][:,:,1] = np.zeros((2,2))        # zero matrix
      # ↓


# Γ[i+1] shape: (2,1,2)
Gamma[i+1] = np.zeros((2,1,2), dtype=complex)
Gamma[i+1][0,0,0] = 0.0       # up
Gamma[i+1][0,0,1] = eta       # up-down
Gamma[i+1][1,0,0] = 1.0       # down-up
Gamma[i+1][1,0,1] = 0.0       # down-down
...
plt.xlabel('Position')
plt.ylabel('Time')
plt.title(r'$\langle S_z\left(x,t\right)\rangle, J_{xy} = $' + f"{Jxy}" + r', $J_z = $' + f"{Jz}" + " Entangled 2-spin excitation")
