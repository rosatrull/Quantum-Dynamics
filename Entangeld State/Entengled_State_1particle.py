...
#eta = np.exp(1j*phi)
eta = 1j
i_assigned = 25  # zero-based index: entangles sites i-1 and i
i = i_assigned - 1

# 1) increase bond dimension between i-1 and i
lambda_list[i] = np.array([1/np.sqrt(2), 1/np.sqrt(2)])

# 2) Γ[i-1] changes to shape (1,2,2)
Gamma[i-1] = np.zeros((1,2,2), dtype=complex)
Gamma[i-1][0,0,0] = 1.0   # up  = [1 0]
Gamma[i-1][0,1,1] = 1.0   # down = [0 1]

# 3) Γ[i] changes to shape (2,1,2)
Gamma[i] = np.zeros((2,1,2), dtype=complex)
Gamma[i][0,0,1] = 1.0     # down = [1 0]^T
Gamma[i][1,0,0] = eta     # up   = [0 η]^T
...
plt.xlabel('Position')
plt.ylabel('Time')
plt.title(r'$\langle S_z\left(x,t\right)\rangle, J_{xy} = $' + f"{Jxy}" + r', $J_z = $' + f"{Jz}" + ', Initial entangled state at position: ' + f"{i_assigned}")
...
"""
# --- ENTANGLEMENT VS TIME FOR DIFFERENT ETA VALUES ----------------------

eta_values = [1, -1, 1j, -1j]
labels = [r'$\eta=1$', r'$\eta=-1$', r'$\eta=i$', r'$\eta=-i$']

# Dictionary to store the averaged entanglement entropy for each eta
avg_S_t_storage = {}

t_start_fix = time.time()

# Loop over the different initial state parameters (eta)
for eta_val, label in zip(eta_values, labels):
    print(f"\n--- Starting simulation for {label} (eta={eta_val}) ---")
    
    # 1. REINITIALIZE THE MPS STATE (essential step)
    lambda_list = [np.array([1], dtype=complex) for _ in range(L+1)]
    Gamma = [np.zeros((1,1,d), dtype=complex) for _ in range(L)]

    for n in range(L):
        Gamma[n][0,0,1] = 1.0  # spin down everywhere

    # Center site index
    i = i_assigned - 1
    
    # Bond i-1 <-> i
    lambda_list[i] = np.array([1/np.sqrt(2), 1/np.sqrt(2)], dtype=complex)
    # Bond i <-> i+1
    lambda_list[i+1] = np.array([1/np.sqrt(2), 1/np.sqrt(2)], dtype=complex)

    # Γ[i-1] shape: (1, 2, 2)
    Gamma[i-1] = np.zeros((1, 2, 2), dtype=complex)
    Gamma[i-1][0,0,0] = 1.0  # up
    Gamma[i-1][0,1,1] = 1.0  # down

    # Γ[i] shape: (2, 2, 2)
    Gamma[i] = np.zeros((2,2,2), dtype=complex)
    # ↑ component
    Gamma[i][:,:,0] = np.sqrt(2) * np.eye(2)
    # ↓ component
    Gamma[i][:,:,1] = np.zeros((2,2))

    # Γ[i+1] shape: (2,1,2)
    Gamma[i+1] = np.zeros((2,1,2), dtype=complex)
    Gamma[i+1][0,0,0] = 0.0
    Gamma[i+1][0,0,1] = eta_val
    Gamma[i+1][1,0,0] = 1.0
    Gamma[i+1][1,0,1] = 0.0

    # 2. REINITIALIZE STORAGE FOR THIS RUN
    S_all_eta = np.zeros((L + 1, n_steps), dtype=float)
    w_acc = 0.0 # Reset accumulated discarded weight
    
    # 3. RUN THE TIME EVOLUTION LOOP AGAIN
    for t_idx in range(n_steps):
        
        # Trotter-Suzuki decomposition steps (2nd order)
        # Half-step on even bonds (indices 1,3,5,...)
        for j in range(1, L - 1, 2):
            Gamma, lambda_list, w_acc = updateBond(Gamma, lambda_list, j, U_half, chi_max, w_acc)

        # Full-step on odd bonds (indices 0,2,4,...)
        for j in range(0, L - 1, 2):
            Gamma, lambda_list, w_acc = updateBond(Gamma, lambda_list, j, U, chi_max, w_acc)

        # Half-step on even bonds again (indices 1,3,5,...)
        for j in range(1, L - 1, 2):
            Gamma, lambda_list, w_acc = updateBond(Gamma, lambda_list, j, U_half, chi_max, w_acc)

        # Measure entanglement entropy
        S_all_eta[:, t_idx] = entanglement_entropy(lambda_list)

    # 4. PROCESS AND STORE RESULTS
    S_t = np.mean(S_all_eta, axis=0)  # average over bonds
    avg_S_t_storage[label] = S_t
    print(f"Simulation for {label} finished. Total time: {time.time() - t_start_fix:.2f} seconds.")


# 5. PLOTTING THE COLLECTED DATA
plt.figure(figsize=(8,6), dpi=500)

for label, S_t in avg_S_t_storage.items():
    plt.plot(time_steps, S_t, label=label)

plt.xlabel(r'Time')
plt.ylabel(r'Average entanglement entropy $S(t)$')
plt.title(r'Entanglement growth for different $\eta$ values')
plt.legend()
plt.grid(False)
plt.show()

print("\nAll simulations and the final comparison plot are complete.")


# 6. PLOTTING THE COLLECTED DATA (Linear vs Log)
# Filter out t=0 for the log plot
non_zero_indices = time_steps > 0
log_time_steps = np.log(time_steps[non_zero_indices])

plt.figure(figsize=(8,6), dpi=500)

for label, S_t in avg_S_t_storage.items():
    # Only take S_t values corresponding to t > 0
    S_t_filtered = S_t[non_zero_indices]
    plt.plot(log_time_steps, S_t_filtered, label=label)

plt.xlabel('ln(Time t)')
plt.ylabel('Average entanglement entropy S(t)')
plt.title('Average Entanglement Growth vs ln(t) (Log-Linear Scale)')
plt.legend()
plt.grid(False)
plt.savefig('average_entanglement_log_linear_scale.png')

print("\\nAll simulations and the final comparison plots are complete.")
"""